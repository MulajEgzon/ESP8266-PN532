...
uint32_t F_CPU = 80000000;
uint16_t sda_bitmask, scl_bitmask, iteration_scl_halfcycle;
uint16_t iteration_remove_spike, iteration_scl_clock_stretch;
...
//  ets_uart_printf("Start of Writing!\n");
	...
	// ets_uart_printf("End of Writing!\n");
	return;
}
...
void brzo_i2c_read(uint8_t *data, uint8_t nr_of_bytes, bool repeated_start) {
	...
	// ets_uart_printf("End Reading\n");

	return;
}

void brzo_i2c_start_transaction(uint8_t slave_address,
		uint16_t SCL_frequency_KHz) {
	// 7 Bit Slave Address; SCL Frequency in Steps of 100 KHz, range: 100 -- 1000 KHz
//	ets_uart_printf("\nSlave_address : 0x%02x \n",slave_address);
...
uint8_t brzo_i2c_end_transaction() {
	...
	i2c_error = 0;
	/* ets_uart_printf("End Transaction\n");
	 ets_uart_printf("i2c_error %u\n",i2c_error);
	 */
	return dummy;
}

void brzo_i2c_setup(uint8_t sda, uint8_t scl,
	...
	if (F_CPU == 160000000) {
		...
		if (clock_stretch_time_out_usec < 100)
			...
			iteration_scl_clock_stretch = 730 * clock_stretch_time_out_usec
					/ 100;
	} else {
		// 80 MHz
		...
		if (clock_stretch_time_out_usec < 100)
			...
			iteration_scl_clock_stretch = 470 * clock_stretch_time_out_usec
					/ 100;
	}
...
//	gpioPinMode(scl, GPIO_SET_PULLUP, GPIO_AS_OUTPUT);
//	gpioPinMode(sda, GPIO_SET_PULLUP, GPIO_AS_OUTPUT);
